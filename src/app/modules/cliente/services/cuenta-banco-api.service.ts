...
export class CuentaBancoApiServices {
  private _generalService = inject(GeneralApiService);
  public arrCuentaBancosSignal = signal<any[]>([]);

  constructor() {}

  lista() {
    return this._generalService
      .consultaApi(`${API_ENDPOINTS.GENERAL.CUENTABANCO.LISTA}`)
      .pipe(
        tap((respuesta: any) =>
          this.arrCuentaBancosSignal.set(respuesta),
        ),
      );
  }
}

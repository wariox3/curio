...
export class TipoPersonaApiServices {
  private _generalService = inject(GeneralApiService);
  public arrTiposPersonasSignal = signal<any[]>([]);

  constructor() {}

  lista() {
    return this._generalService
      .consultaApi(`${API_ENDPOINTS.GENERAL.TIPOPERSONA.LISTA}`)
      .pipe(
        tap((respuesta: any) =>
          this.arrTiposPersonasSignal.set(respuesta),
        ),
      );
  }
}
